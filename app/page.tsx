...
import { ArrowUpRight, Check, Mail, Menu, Quote } from "lucide-react"
    ...
    <main>
      ...
      <section id="top" className="hero-section relative flex min-h-[100svh] items-end overflow-hidden px-6 pb-16 pt-28 lg:px-10 lg:pb-16">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl">
          <p className="eyebrow mb-7 font-bold text-paper/75"><span className="mr-2 inline-block size-2 rounded-full bg-accent" />SEO content writer &amp; analyst</p>
      ...
      <section className="border-b border-line bg-paper-deep px-6 py-12 lg:px-10 lg:py-16"><div className="mx-auto max-w-4xl text-center"><Quote className="mx-auto size-7 text-accent" /><blockquote className="mt-6 font-serif text-3xl leading-tight text-ink sm:text-4xl lg:text-5xl">&ldquo;The clearest answer is usually the one people remember.&rdquo;</blockquote><p className="mt-6 text-sm text-ink-muted">A principle behind every content decision</p></div></section>

      <section id="services" className="mx-auto max-w-7xl px-6 py-14 lg:px-10 lg:py-20"><div className="grid gap-8 lg:grid-cols-[0.7fr_1.3fr] lg:gap-16"><div><p className="eyebrow">What I do</p><h2 className="mt-5 max-w-sm font-serif text-5xl leading-[0.98] tracking-tight text-ink lg:text-6xl">Clarity is a competitive advantage.</h2><p className="mt-6 max-w-xs leading-7 text-ink-muted">Useful content starts with understanding the audience, the business, and the decision between them.</p></div><div className="divide-y divide-line">{services.map((service) => <article key={service.title} className="py-5 first:pt-0"><div><h3 className="font-serif text-3xl text-ink">{service.title}</h3><p className="mt-3 max-w-xl leading-7 text-ink-muted">{service.text}</p></div></article>)}</div></div></section>
