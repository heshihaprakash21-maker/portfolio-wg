...
export default function Page() {
  return (
    <main>
      ...
      <section id="contact" className="bg-accent px-6 py-20 text-paper lg:px-10 lg:py-28"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[1.1fr_0.9fr] lg:items-end"><div><p className="eyebrow text-paper/70">Ready when you are</p><h2 className="mt-5 max-w-3xl font-serif text-6xl leading-[0.9] tracking-tight lg:text-8xl">Let&apos;s make your next piece easier to find.</h2><div className="mt-10 flex flex-wrap items-center gap-6"><a href={`mailto:${persona.email}`} className="inline-flex items-center gap-2 text-lg underline underline-offset-8">{persona.email} <ArrowUpRight className="size-5" /></a><a href="https://linkedin.com/in/heshiha-thangamani-7581a0193" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-lg underline underline-offset-8"><ArrowUpRight className="size-5" /> LinkedIn</a></div></div><div className="border-t border-paper/40 pt-6 lg:border-l lg:border-t-0 lg:pl-10 lg:pt-0"><p className="max-w-sm text-lg leading-8 text-paper/80">Tell me what you&apos;re working on, where it feels stuck, and what a useful outcome would look like.</p><a href={`mailto:${persona.email}?subject=Project%20inquiry%20for%20Heshiha`} className="mt-8 inline-flex items-center gap-3 rounded-full bg-paper px-6 py-3 text-sm font-medium text-ink transition-transform hover:-translate-y-0.5">Start with an email <ArrowUpRight className="size-4" /></a><p className="mt-5 text-sm text-paper/60">No long form. No jargon. Just a clear first conversation.</p></div></div></section>

      <footer className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-8 text-sm text-ink-muted md:flex-row md:items-center md:justify-between lg:px-10"><p>© 2026 {persona.name}</p><div className="flex flex-wrap gap-6"><a href="#top" className="hover:text-ink">Back to top</a><a href="https://linkedin.com/in/heshiha-thangamani-7581a0193" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-ink"><ArrowUpRight className="size-4" /> LinkedIn</a><a href={`mailto:${persona.email}`} className="inline-flex items-center gap-2 hover:text-ink"><Mail className="size-4" /> Email me</a></div></footer>
    </main>
  )
}
